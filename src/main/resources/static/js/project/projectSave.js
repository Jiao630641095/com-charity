layui.use(["form", "layer", "layedit", "jquery","upload"], function () {
    ...
    //创建一个编辑器
    var editIndex = layedit.build('project_content');

    //保存新闻
    form.on("submit(save_button)", function (data) {
        $("#websiteUrl").val($("select[name='websiteUrl']").val());
        //显示、审核状态
        var project = {
            id: $("#id").val(),
            name:$(".name").val(),
            isShow: data.field.show,
            initiator: $(".newsAuthor").val(),
            websiteUrl: $("#websiteUrl").val(),//$("select[name='websiteUrl']").val(),
            sketch: $("#sketch").val(),
            content: layedit.getContent(editIndex),
            imgSrc:$("#imgUrl").attr("src")
        };
        $.post(
            "/admin/project/save",
            project,

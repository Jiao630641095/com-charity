layui.use(["form","layer","jquery","laypage","laydate"],function(){
	...
	//加载页面数据
	var standardData = '';
	$.get("/tpcstandard/list", function(data){
		standardData = data;
		standardList();//执行加载数据的方法
	},"json")
	...
	//编辑政策
	$("body").on("click",".tpcstandard_edit",function(){  //编辑
		var id = $(this).parent().parent().children("td:first-child").children("input:first-child").val();
		sessionStorage.setItem("tpcstandardID",id);
		var index = layui.layer.open({
			title : "编辑政策",
			type : 2,
			content : "/tpcstandard/goupdate",
			success : function(){
				layui.layer.tips('点击此处返回政策列表', '.layui-layer-setwin .layui-layer-close', {
					tips: 3
				});
	...
	//查看
	$("body").on("click",".tpcstandard_look",function(){  //查看
		var id = $(this).parent().parent().children("td:first-child").children("input:first-child").val();
		sessionStorage.setItem("tpcstandardID",id);
		var index = layui.layer.open({
			title : "政策详情",
			type : 2,
			content : "/tpcstandard/tpcStandardLook",
			success : function(){
				layui.layer.tips('点击此处返回政策列表', '.layui-layer-setwin .layui-layer-close', {
					tips: 3
				});
    ...
    $("body").on("click",".search_button",function(){
       var standard = {
		...
		$.post("/tpcstandard/getStandardListWhere",standard, function(data){
			standardData = data;
			standardList();//执行加载数据的方法
		},"json")
    });
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
                    dataHtml +=
						...
						+'<td align="center">'
						+  '<a  shiro:hasPermission="tpcstandard:view" class="layui-btn layui-btn-mini tpcstandard_look"><i class="layui-icon">&#xe615;</i> 查看</a>'
						+  '<a  shiro:hasPermission="tpcstandard:update" class="layui-btn layui-btn-mini tpcstandard_edit"><i class="iconfont icon-edit"></i> 编辑</a>'
						+'</td>'
						+'</tr>'
				}
			}else{
